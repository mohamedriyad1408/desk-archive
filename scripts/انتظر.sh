...
#
# حدود السلاح (ق-٠٠٤/ق-٠٠٥): التسليح يعيش ما عاش الدور جالسًا على انتظاره.
# عملية خلفية بعد انتهاء استجابة الدور رصدٌ آلي بلا حكم — لا تُحتسب حياة دور.
# أول فعل بعد أي يقظة (وخاصة على مساحة أُعيد تجهيزها): قِس سلاحك (kill -0
# بناتج أمر وطابع زمني)؛ ملف سلاح يتيم دليل موت: نظّف وأعد تسليحًا جديدًا
# بعد فتح.sh. المدد تُحسب من طوابع git لا من ساعة الجلسات.
set -euo pipefail
cd "$(dirname "$0")/.."
